...
def render_cover(s):
    """Cover slide — data-driven from slide.title, .subtitle, .tag (with optional splash)."""
    audio = f"audio/slide_{s['n']:02d}.mp3"
    title = s.get("title", "BiRAGAS")
    subtitle = s.get("subtitle", "")
    tag = s.get("tag", "")
    words = title.strip().split()
    if len(words) <= 1:
        title_html = f'<span class="splash">{esc(title)}</span>'
    elif len(words) == 2:
        title_html = f'{esc(words[0])}<br><span class="splash">{esc(words[1])}.</span>'
    else:
        head = " ".join(words[:-1])
        title_html = f'{esc(head)}<br><span class="splash">{esc(words[-1])}.</span>'
    sub_html = f'<div class="sub">{esc(subtitle)}</div>' if subtitle else ""
    tag_html = f'<span>{esc(tag)}</span>' if tag else ""
    eyebrow = f"Ayass BioScience · {esc(s.get('stamp','BiRAGAS Platform · MMXXVI'))}"
    return f'''<section class="slide active" data-audio="{audio}" data-label="{esc(title)}">
  <div class="spread cover">
    ...
    {sub_html}
    <div class="meta-row">
      <span class="pill red">{esc(s.get('stamp','BIRAGAS'))}</span>
      {tag_html}
    </div>
  </div>
</section>'''
...
def render_close(s):
    """Final slide — dark close-slide layout, data-driven from slide.title + .subtitle."""
    audio = f"audio/slide_{s['n']:02d}.mp3"
    ...
    if len(parts) >= 2:
        ...
        line2_text = ". ".join(parts[1:]).strip().rstrip(".")
        line2 = f'{esc(line2_text)}<span class="period"></span>'
    else:
        line1 = esc(title.rstrip(".") + ".")
        line2 = ""
    line2_html = f'<div class="manifesto next">{line2}</div>' if line2 else ""
...
def render_framework(s):
    """Cards grid — data-driven from slide.title and slide.wrongs[]."""
    audio = f"audio/slide_{s['n']:02d}.mp3"
    cards = ""
    items = s.get("wrongs", [])
    for w in items:
    ...
    grid_class = "cards-3" if len(items) <= 3 else "cards-5"
    eyebrow = s.get("eyebrow") or "— " + esc(s.get("stamp", "")).title() + " —"
...
def build():
    ...
    sections_html = "\n".join(sections)
    # derive masthead branding from deck-level fields in slides.json
    deck_title = SLIDES.get("title", "BiRAGAS")
    deck_brand_tag = deck_title.split("·")[-1].strip().upper() if "·" in deck_title else deck_title.upper()
    deck_byline = SLIDES.get("byline", "AYASS BIOSCIENCE").upper()
    page_title = f"{deck_title} · Ayass BioScience · MMXXVI"
    out = (
        TEMPLATE
        .replace("{{SECTIONS}}", sections_html)
        .replace("{{TOTAL}}", str(len(SLIDES["slides"])))
        .replace("{{BRAND_TAG}}", deck_brand_tag)
        .replace("{{BYLINE}}", deck_byline)
        .replace("{{PAGE_TITLE}}", page_title)
    )
    (BASE / "index.html").write_text(out)
    print(f"Wrote index.html with {len(SLIDES['slides'])} sections · brand_tag={deck_brand_tag!r}")
...
.cards-5{display:grid;grid-template-columns:repeat(5,1fr);gap:14px;flex:1;align-content:start}
.cards-5 .card{padding:18px 16px}
.cards-5 .card .num{font-size:42px;margin-bottom:8px}
.cards-5 .card .card-label{font-size:11px;letter-spacing:0.18em;margin-bottom:8px}
.cards-5 .card .card-desc{font-size:14px;line-height:1.32}
@media (max-width:1100px){.cards-5{grid-template-columns:repeat(3,1fr)}.cards-5 .card:nth-child(4),.cards-5 .card:nth-child(5){grid-column:span 1}}
@media (max-width:700px){.cards-5{grid-template-columns:1fr}}
...
<div class="deck">
  <header class="masthead">
    ...
    <div class="meta">MMXXVI · {{TOTAL}} SLIDES · {{BYLINE}}</div>
    <div class="controls">
